...
const getGames = async (req, res, next) => {
  try {
    ...
    const apiGames = name
      ? await searchApiGames(name)
      : await getManyApiGames();

    res.json(
      !name
        ? [...apiGames, ...dbGames.map((game) => toCard(game))]
        : [...dbGames.map((game) => toCard(game)), ...apiGames].slice(0, 15)
    );
  } catch (e) {
...
const postGame = async (req, res, next) => {
  try {
    ...
    await game.addGenres(
      idGenres.map((genre) => genre.dataValues.id),
      { through: Videogenre }
    );
    await game.addPlatforms(
      idPlatforms.map((platform) => platform.dataValues.id),
      { through: Videoplatform }
    );

    const createdGame = await Videogame.findByPk(game.id, {
      include: [{ model: Genre }, { model: Platform }],
    });
